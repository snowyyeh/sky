module.exports = {
    run: async (client, msg, args) => {
        const r = client.db;
        if (!args[0]) {
            var points = await r.table('points').get(msg.author.id).run();
            if (!points) {
                ...
                points = await r.table('points').get(msg.author.id).run();
            ...
            var points = await r.table('points').get(user.id).run();
            if (!points) {
                ...
                points = await r.table('points').get(user.id).run();
